...
const TikTokStats = () => {
  ...
  const accessToken = localStorage.getItem("tiktokAccessToken"); // Giả định lưu token
  ...
  useEffect(() => {
    const fetchTikTokVideos = async () => {
      try {
        ...
        const fields = "id,create_time,video_description,share_count,view_count,like_count,commentCount";
        ...
        // Định dạng dữ liệu theo cấu trúc TikTokVideo
        const videoList: TikTokVideo[] = data.videos.map((item: any) => ({
          id: item.id,
          title: item.description || "No description",
          thumbnail: item.cover_image || "/placeholder.svg",
          publishedAt: item.created_at,
          viewCount: item.stats.play_count || 0,
          likeCount: item.stats.like_count || 0,
          commentCount: item.stats.comment_count || 0,
        }));
        ...
        console.error(err);
        setError("An error occurred while fetching TikTok videos.");
        setLoading(false);
      }
    };
